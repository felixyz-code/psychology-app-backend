...
| GET | `/organizations/:organizationId/settings` | Organizations | Yes | `organization.read` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; returns explicit ABSENT/PRESENT state and effective 60-minute fallback | 404 / 403 | Implemented | 5.2 runtime |
| PATCH | `/organizations/:organizationId/settings` | Organizations | Yes | `organization.manage` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; exact absent/present compare-and-swap required | 404 / 400 / 409 | Implemented | 5.2 runtime |
| GET | `/organizations/:organizationId/branding` | Organizations | Yes | `organization.read` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; returns explicit ABSENT/PRESENT state | 404 / 403 | Implemented | 5.2 runtime |
| PATCH | `/organizations/:organizationId/branding` | Organizations | Yes | `organization.manage` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; strict bounded primaryColor and compare-and-swap required | 404 / 400 / 409 | Implemented | 5.2 runtime |
| GET | `/organizations/:organizationId/logo` | Organization logo | Yes | `organization.read` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; singleton metadata with ABSENT/PRESENT state | 404 / 403 | Implemented | 5.3 runtime |
| GET | `/organizations/:organizationId/logo/content` | Organization logo | Yes | `organization.read` | Path must match selected tenant; private protected stream with no static/public bypass | 404 / 403 | Implemented | 5.3 runtime |
| PUT | `/organizations/:organizationId/logo` | Organization logo | Yes | `organization.manage` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; bounded multipart image and exact first/replace CAS | 404 / 400 / 409 / 413 | Implemented | 5.3 runtime |
| DELETE | `/organizations/:organizationId/logo` | Organization logo | Yes | `organization.manage` | Path must match selected tenant; allows `ACTIVE` or `SUSPENDED`; exact present-row CAS and best-effort blob cleanup | 404 / 400 / 409 | Implemented | 5.3 runtime |
